...
module.exports.getContestById = async (req, res, next) => {
  try {
    let contestInfo = await db.Contests.findOne({
      where: { id: req.headers.contestId }, //констетсАйди с большой буквы
      order: [
        [db.Offers, 'id', 'asc'],
      ],
      include: [
...
module.exports.getOfferFiles = async (req, res, next) => {
  try {
    const { body: { queryFilter } } = req;                                                  //поулчить файлы офера, создать фильр. одно из услови - имя файла, проверить бади.  резульаьб - выдать файлы( res send result)
...
module.exports.setNewOffer = async (req, res, next) => {
  ...
  try {
    const result = await contestQueries.createOffer(obj); //поменял лет на конст в резалте
